...
#def train_prototxt_maker(train, solver, train_set):
...
def train_runner(solver, train_set, model):
    solstate = 'resnet_db/model/vgg128_noup/train_iter_30000.solverstate'
    cmd = os.environ['CAFFE_DIR'] + os.environ['CAFFE_BIN'] + ' train' \
    ' --solver=' + os.environ['CONFIG_DIR'] + '/' + solver + '_' + train_set + '.prototxt' \
    ' --snapshot=' + solstate + ' --gpu=' + '2' #os.environ['DEV_ID'] # ##change solver ' --weights=' + model + ' --snapshot=' + solstate
    print 'Running ' + cmd
