	    ...
	    #thresholding
            ...
            for j in range(0,bottom[0].data.shape[1]):
				if bottom[5].data[i, j] == 0.0: #(np.max(bottom[0].data[i, j, ...]) < 0.0):
					...
					#aux2 = np.zeros(shape=(14*14), dtype=np.float32)
					#aux4 = np.zeros(shape=(14,14), dtype=np.float32)
					#auxi = np.copy(aux3.flatten())
					#idx = []
					#idx = np.argwhere(auxi > 0.2*np.max(aux3))
					
					#aux2[idx] = 1.0
					#aux4 = np.reshape(aux2, (14,14))
					#aux3[aux4 == 1.0] = 1.0
					
					#top[0].data[i, j+1, ...] = aux3 #
					
					
					
					#aux_ctx = np.copy(top[0].data[i, j+1, ...])
					#aux_ctx_1 = scipy.misc.imresize(aux_ctx, (15,15))
					#print aux_ctx_1
					#aux_ctx_avg = scipy.misc.imresize(aux_ctx, (15,15))
					#for he in range(1,14):
					#	for wi in range(1,14):
					#		aux_ctx_avg[he, wi] = (aux_ctx_1[he-1, wi] + aux_ctx_1[he-1, wi-1] + aux_ctx_1[he-1, wi+1] + aux_ctx_1[he, wi] + aux_ctx_1[he, wi-1] + aux_ctx_1[he, wi+1] + aux_ctx_1[he+1, wi] + aux_ctx_1[he+1, wi-1] + aux_ctx_1[he+1, wi+1])/9.0

					#print aux_ctx_avg
					#top[0].data[i, j+1, ...] = scipy.misc.imresize(aux_ctx_avg, (14,14))
