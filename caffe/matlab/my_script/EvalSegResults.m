...
if is_server
  if strcmp(dataset, 'voc12_weakly')
    VOC_root_folder = '/usr/data/datasets1/voc/voc2012/VOCdevkit';    
...
output_mat_folder = fullfile('/home/carolina/projects/weakly_project_original/deeplab/caffe', dataset, feature_name, model_name, testset, 'crf');
...
if is_mat
  ...
  for i = 1 : numel(output_dir)
    ...
    %raw_result = imresize(raw_result, [1700, 800], 'nearest');
    ...
    if strcmp(dataset, 'voc12_weakly')
      ...
      %img = imread(fullfile(VOC_root_folder, 'test', [img_fn, '.png']));
      %img = imread(fullfile(VOC_root_folder, 'images', [img_fn, '.png']));
    elseif strcmp(dataset, 'coco')
